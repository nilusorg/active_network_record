...
module ActiveNetworkRecord
  class Base
    ...
    def update(args = {})
      return create(args) unless persisted?
      ...
      if response.parsed_response
        ...
        when 200
          reset_cache!
          self.class.find(response.parsed_response['id'] || response.parsed_response.dig(self.class.name.demodulize.underscore.singularize.to_s, 'id'))
        when 422
    ...
    def build_single_new_url
      "#{self.class.service_path}/#{self.class.name.demodulize.underscore.pluralize}"
    end

    def build_single_url
      "#{build_single_new_url}/#{id}.json"
    end
    ...
    class << self
      ...
      def relation(options)
        const_set "Relation".to_sym, Class.new(ActiveNetworkRecord::Base::Relation) if self::Relation == ActiveNetworkRecord::Base::Relation
